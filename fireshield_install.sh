...
def _verify():
 ...
 marker="# __FIRESHIELD_CODE_START__\n"
 idx=src.find(marker)
 if idx==-1: print("FireShield: archivo corrupto",file=sys.stderr); sys.exit(1)
 code=src[idx+len(marker):]
 expected="405795ad37950881d329fbd68af6d98a8b343186aad191b5b82add0f5f5def86"
 actual=hashlib.sha256(code.encode()).hexdigest()
 if actual!=expected: print("\033[91mFireShield: INTEGRIDAD COMPROMETIDA\033[0m\nEl archivo fue modificado. Reinstala desde el instalador oficial.",file=sys.stderr); sys.exit(1)
_verify()
# __FIRESHIELD_CODE_START__
...
def run_priv(cmd):
 ...
 except FileNotFoundError: return False,"pkexec no encontrado"
...
# Proteger archivo contra modificaciones
sudo chmod 444 /usr/share/fireshield/fireshield.py
sudo chattr +i /usr/share/fireshield/fireshield.py 2>/dev/null || true
echo "   Codigo protegido contra modificaciones"
...
# FireShield - solo lectura sin contrasena, acciones requieren pkexec
$CURRENT_USER ALL=(ALL) NOPASSWD: $UFW_PATH status
$CURRENT_USER ALL=(ALL) NOPASSWD: $UFW_PATH status verbose
$CURRENT_USER ALL=(ALL) NOPASSWD: $UFW_PATH status numbered
SUDOEOF
